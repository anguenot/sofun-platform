...
import org.sofun.core.team.table.TeamTournamentStageRankingTableImpl;
 ...
 * 
 */
public class TestTournamentSeasonRankingTable extends SofunCoreTestCase {

    public TestTournamentSeasonRankingTable(String testName) {
        super(testName);
    }
    ...
    public void testAddTeamTournamentStageRankingTableEntry()
        ...
        TournamentStage stage = new TournamentStageImpl(1);

        TeamTournamentStageRankingTable stageTable = new TeamTournamentStageRankingTableImpl(
                t, stage);

        assertEquals(stage, stageTable.getTournamentStage());
        assertEquals(t, stageTable.getTeam());

        assertEquals(2, stageTable.getEntries().size());
        assertEquals(2, t.getRankingTable().getEntries().size());

        for (MemberRankingTableEntry e : stageTable.getEntries()) {
